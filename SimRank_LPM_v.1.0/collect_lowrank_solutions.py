...
def getsolution(taskname, ranks, solver, maxiter):
	...
	for rank in ranks:
		...
		S = sm.launch(args)
		np.save(
		f"{path}/S_{args['taskname']}_rank_{rank}_c_{args['c']}_solver_{solver}_{dt.datetime.now().strftime(dateformat)}.npy",S)
...
if __name__ == "__main__":
	...
	parser = argparse.ArgumentParser("collect_lowrank_solutions.py")
