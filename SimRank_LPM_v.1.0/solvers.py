...
def LSq(beta, H):
	...
	# y = np.linalg.inv(H.T@H)@H.T@b #pinv from normal eq. 
	# y = np.linalg.pinv(H)@(e1*beta) #numpy svd-based pinv.
	Q, R = np.linalg.qr(H) #LSq solution using QR as Hessenberg matrix is non-singular in GMRES
