...
def GetMatrix(path, showfig, delimeter = '\t', savecoo=False):
	...
	if savecoo:
		np.save("data/wikivote_data.npy", np.array(data))
		np.save("data/wikivote_row.npy", np.array(row))
		np.save("data/wikivote_col.npy", np.array(col))
	print(f"Constructing completed. nnz = {A_coo.nnz}.")
	return A_coo

def LoadMatrix(n):
	return scsp.coo_matrix((np.load("data/wikivote_data.npy"), (np.load("data/wikivote_row.npy"), np.load("data/wikivote_col.npy"))), shape=(n,n))
