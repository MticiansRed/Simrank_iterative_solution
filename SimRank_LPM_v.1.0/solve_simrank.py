...
dateformat = "%Y-%m-%d-%H-%M" #for log and plots saving
...
def Solve(acc, m_Krylov, rank, k_iter_max, taskname, A, c, solver, optimize): #solvers = list of flags: ['SimpleIter, GMRES, MinRes'] (in any order)
	...
	def AltMin_LSFP(): #AltMin with Least Squares Fixed Point
	...
	def QMin_Newton():
		...
		ts = time.time()
		U, solutiondata = optslv.Newton(A_csr, c, rank, maxiter=k_iter_max, gmres_restarts=1, m_Krylov=m_Krylov, solver=optslv.GMRES_scipy, stagstop=1e-5, optimize=optimize, compute_functional=False)
		M = U@U.T
