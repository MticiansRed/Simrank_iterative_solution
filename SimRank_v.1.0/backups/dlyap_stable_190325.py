...
def SimpleIterIP(k_max, A, c, tau, N, eps = 1e-13): #in most models c = 0.8 and k_max = 5
	I = np.identity(N)
	A = csr_matrix(A)
	st = time.time()
	S = I
...
def Arnoldi(V_list, h, m, A, c, N):
	for j in range(m):
		print(f"Building Arnoldi: V[:,{j}]", end = '\r')
		v_j = V_list[j]
		w_j = colvecto1dim(G(v_j, A, c, N))
		for i in range(j):
			v_i = V_list[i]
			h[i,j] = v_i@w_j 
			w_j = w_j - h[i,j]*v_i
...
def GMRES_m(k_max, m, A, c, N, eps = 1e-13): #Planned changes: 1. Remove x_0 from the body. 2. Move G to func arg.
	iterations = []
	residuals = []
	st = time.time()
	...
	for k in range(k_max):
		...
		print("Relative residual:", residual/np.linalg.norm(G(s, A, c, N), ord = 2))
		residuals.append(residual)
		if (residual < eps):
			break
		
		beta = np.linalg.norm(r, ord = 2)
		V_list = [np.array([0.0]*N**2)]*(m+1)
		V_list[0] = colvecto1dim(r)/beta
		H = np.zeros((m+1, m))
		st = time.time() ###
		m = Arnoldi(V_list, H, m, A, c, N)
